...
class UserStore {
  ...
  @action SignIn = async (login: IUserLogin) => {
    this.IsLoading = true;
    let data: any = await agend.Account.Login(login);
...
if(remember){
  ...
  @action SignOut = async () => {
    await agend.Account.SignOut();
    this.UserData = null;
    this.IsLogin = false;
  };
  @action SignUp = async (register: IRegister) => {
    let data = await agend.Account.Register(register);
    if (data.status) {
      runInAction(() => {
        this.IsError = data.status;
      });
  ...
  @action SendFriendRequest = async (id: string) => {
    let status = await agend.Account.SendFriendRequest(id, this.UserData?.id!);
  };
  @action AcceptFriendRequest = async (i: IUserDTOs) => {
    await agend.Account.AcceptFriendRequest(i.id, this.UserData?.id!);
    let newUserData: IUserDTOs = {
      ...
      userFriendsRequests: this.UserData?.usersFriends.filter(
        (r) => r.id !== i.id
      )!,
    };
  ...
  @action SearchUserByUserName = async (userName: string) => {
    this.SearchResult = await agend.Account.SearchUsersByUserName(userName);
  };
  ...
  @action SetAvatar = async (avatar: any) => {
    let newAvatar = await await agend.Account.SetAvatar(
      this.UserData?.id!,
      avatar
    );
