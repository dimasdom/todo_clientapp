...
class TodoItemsStore {
  ...
  @action GetTodoLists = async (id: string | undefined) => {
    if (this.rootStore.todoLists.TodoListsMapped.get(id!)?.common) {
      this.IsLoading = true;
      let data = await agend.TodoItem.GetTodoItems(id);
      this.SetItems(data);
  ...
  @action DeleteTodoItem = async (id: string | undefined) => {
    let status = await agend.TodoItem.DeleteTodoItem(id);
  ...
  @action ChangeDoneStatus = async (id: string | undefined) => {
    let status = await agend.TodoItem.SetDoneStatus(id);
    let todoItem = this.TodoItems?.filter((i) => i.id === id);
    if (todoItem !== undefined) {
      todoItem[0].done = todoItem[0].done ? false : true;
      this.TodoItems =
        this.TodoItems?.filter((i) => i.id !== id) === undefined
          ? []
          : this.TodoItems?.filter((i) => i.id !== id);
  ...
  @action ChangeDescription = async (id: string | undefined, desc: string) => {
    let status = await agend.TodoItem.ChangeTodoItem(id, desc);
    let todoItem = this.TodoItems?.filter((i) => i.id === id);
    if (todoItem !== undefined) {
  ...
  @action CreateTodoItem = async (
    todoListId: string | undefined,
    desc: string
  ) => {
    if (this.HubConnection) {
      try {
        await this.HubConnection.invoke("CreateTodoItem", {
          TodoListId: todoListId,
          Description: desc,
          CreatedByUserId: this.rootStore.user.UserData?.id,
        });
      } catch {
      ...
      if (todoListId) {
        ...
        let response = await agend.TodoItem.CreateTodoItem(todoItem);
